...
class App extends Component {
  ...
  performSearch = (e) => {
    ...
    const requestUrl = `https://api.themoviedb.org/3/search/multi?query=${val}&api_key=${this.apiKey}`;
  ...
  render() {
    return (
			<div className="UserProfile">
				<div className="User">
					<div className="name">Bryan V</div>
...
const TitleList = (props) => {
  ...
  const loadContent = () => {
    const requestUrl = `https://api.themoviedb.org/3/${props.url}&api_key=${apiKey}`;
  ...
  useEffect(() => {
    loadContent();
  }, [props.url]); // Load content when props.url changes
  ...
  const renderTitles = () => {
    if (data.results) {
      return data.results.map((title, i) => {
        if (i < 5) {
